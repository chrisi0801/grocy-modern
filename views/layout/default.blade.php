...
<head>
	...
	<link href="{{ $U('/css/grocy.css?v=', true) }}{{ $version }}"
		rel="stylesheet">
	<link href="{{ $U('/css/grocy_theme.css?v=', true) }}{{ $version }}"
		rel="stylesheet">
	<link href="{{ $U('/css/grocy_menu_layout.css?v=', true) }}{{ $version }}"
		rel="stylesheet">
	<link href="{{ $U('/css/grocy_mobile.css?v=', true) }}{{ $version }}"
		rel="stylesheet">
...
<body class="fixed-nav @if(boolval($userSettings['night_mode_enabled_internal'])) night-mode @endif @if($embedded) embedded @endif @if(GROCY_AUTHENTICATED && !$embedded) has-bottom-nav @endif">
	@if(!$embedded)
	<nav id="mainNav"
		class="navbar navbar-light fixed-top app-topbar">
		@if(GROCY_AUTHENTICATED)
		<button id="nav-drawer-toggle"
			type="button"
		...
		<ul class="navbar-nav topbar-actions">
			...
			<li class="nav-item dropdown">
				<a class="nav-link dropdown-toggle discrete-link @if(!empty(GROCY_USER_PICTURE_FILE_NAME)) py-0 @endif"
					...
					<img class="rounded-circle"
						...
						loading="lazy">
					@endif
					<span class="topbar-action-label">{{ GROCY_USER_USERNAME }}</span>
				</a>
			...
			<li class="nav-item dropdown">
				<a class="nav-link dropdown-toggle discrete-link"
					href="#"
					data-toggle="dropdown"><i class="fa-solid fa-sliders-h"></i> <span class="topbar-action-label">{{ $__t('View settings') }}</span></a>
			...
			<li class="nav-item dropdown">
				<a class="nav-link dropdown-toggle discrete-link"
					href="#"
					data-toggle="dropdown"><i class="fa-solid fa-wrench"></i> <span class="topbar-action-label">{{ $__t('Settings') }}</span></a>
